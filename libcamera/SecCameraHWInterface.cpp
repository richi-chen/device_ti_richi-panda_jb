...
void CameraHardwareSec::initDefaultParameters(int cameraId)
{
    ...
    mSecCamera->getExposureState(&exposure_enable, &exposure_default, &exposure_min, &exposure_max, &exposure_step);
    if(exposure_enable)
    {
        char temp_str[30];
...
callbacks:
    ...
    if (mMsgEnabled & CAMERA_MSG_PREVIEW_FRAME) {
        ...
        if (!strcmp(preview_format, CameraParameters::PIXEL_FORMAT_YUV422I)) {
            ...
            char *vu = ((char *)mPreviewHeap->data) + offset;
            yuyv422_to_yuv420sp((unsigned char*)mSecCamera->m_preview_buf[index].start, (unsigned char*)vu, width, height);
            ...
            char *vu = ((char *)mPreviewHeap->data) + offset;
            yuyv422_to_yuv420((unsigned char*)mSecCamera->m_preview_buf[index].start, (unsigned char*)vu, width, height);
         }
    ...
    if (mRecordRunning == true) {
        ...
        if (mMsgEnabled & CAMERA_MSG_VIDEO_FRAME) {
            mDataCbTimestamp(timestamp, CAMERA_MSG_VIDEO_FRAME, mRecordHeap, video_index, mCallbackCookie);
        } else {
...
int CameraHardwareSec::pictureWrapperThread()
{
	return takePicture();
}
...
//    sp<MemoryBase> buffer = new MemoryBase(mRawHeap, 0, mPostViewSize + 8);
        ...
        mParameters.getInt(CameraParameters::KEY_JPEG_THUMBNAIL_HEIGHT) >0)
    {
        ...
        scaleDownYuv422((char*)phyAddr, mPictureViewWidth, mPictureViewHeight, (char*)src, thumbnailWidth, thumbnailHeight);
	...
	LOGW("\n\n\n###############KEY_GPS_TIMESTAMP is =%s\n\n\n", mParameters.get(CameraParameters::KEY_GPS_PROCESSING_METHOD));
        ...
        memcpy(temp_value + sizeof(ExifAsciiPrefix), mParameters.get(CameraParameters::KEY_GPS_PROCESSING_METHOD), (100 - sizeof(ExifAsciiPrefix)));
    ...
    //##############################//
    LOGW("%s %d. data=%x, phyAddr=%x, size =%d", __FUNCTION__, __LINE__, mRawHeap->data, phyAddr, jpeg_size);
...
status_t CameraHardwareSec::takePictureWrapper()
{
...
status_t CameraHardwareSec::setParameters(const CameraParameters& params)
{
    ...
    if (0 < new_preview_width && 0 < new_preview_height &&
        ...
        mSecCamera->getPreviewSize(&current_preview_width, &current_preview_height, &current_frame_size);
        ...
        if (current_preview_width != new_preview_width ||
            ...
            if (mSecCamera->setPreviewSize(new_preview_width, new_preview_height, new_preview_format) < 0) {
                ...
                if (mPreviewWindow) {
                    ...
                    LOGV("%s: mPreviewWindow->set_buffers_geometry (%p)", __func__, mPreviewWindow->set_buffers_geometry);
        ...
        LOGE("%s: Invalid preview size(%dx%d)", __func__, new_preview_width, new_preview_height);
     ...
     */
    if ( !strstr(mParameters.get(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES), support_frame_rate) ) {
        LOGE("ERR(%s):Fail on set FPS. Support=%s, Set FPS = %d", __func__, mParameters.get(CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES), new_frame_rate);
        ret = UNKNOWN_ERROR;
    }
    else {
...
{
    LOGW("%s", __func__);
    //This is a wrapper, it will create a thread, in thread to stoppreview, and take picture
    return obj(dev)->takePictureWrapper();
}
...
static int HAL_getNumberOfCameras()
{
    ...
    numCamera = find_camera();

    LOGI("\n\n\nHave %d Camera devices\n\n\n", numCamera);

    //we must return 1 here. Camera service will check this on its first Ref.
    return numCamera;
}
