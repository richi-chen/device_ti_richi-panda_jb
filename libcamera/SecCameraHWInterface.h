...
public:
    ...
    virtual status_t    takePictureWrapper();
...
private:
    ...
    class PictureWrapperThread : public Thread {
	    CameraHardwareSec *mHardware;
    public:
	    PictureWrapperThread(CameraHardwareSec *hw):
	    Thread(false),
	    mHardware(hw) { }
	    virtual bool threadLoop() {
		    mHardware->pictureWrapperThread();
		    return false;
	    }
    };

    sp<PictureWrapperThread>    mPictureWrapperThread;
    int    pictureWrapperThread();
